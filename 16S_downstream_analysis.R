## Code for processing 16S rRNA gene metabarcoding data (V4) from ISLAND-WATER072024
...
## Community composition Class level plots
# Maja Mucko
...
# Load 16S taxonomy file - assigned with new GG2 database release (V 2022.10)
taxonomy <- read_qza(file="data/WATER_16S_taxonomy_gg2.qza")
...
# Filter out unwanted groups: Archaea, Eukaryota, Chloroplast, Mitochondrium, Plastid, and unassigned Phylum
ps_new <- subset_taxa(ps, 
                           !Domain %in% c("Archaea", "Eukaryota") & # Remove Archaea and Eukaryota
                             !grepl("Chloroplast", Order, ignore.case = TRUE) & # Remove Chloroplast
                             !grepl("Mitochondrium", Family, ignore.case = TRUE) & # Remove Mitochondrium
                             !grepl("Plastid", Family, ignore.case = TRUE) & # Remove Plastid
                             !is.na(Phylum)) # Remove unassigned Phylum
...
saveRDS(ps_rare, file = "data/phyloseq_16S_rare.rds")
asv_table_filtered_rarefied <- as.data.frame(t(otu_table(ps_rare)))
write.table(asv_table_filtered_rarefied, file = "data/Filtered_B_ASV_table_7509.tsv", sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)

# Rarefaction curves for 16S - Figure S1, panel Bacterial community
...
rare_16S <- ggrare(ps_filt, step = 100, plot = FALSE, parallel = FALSE, se = FALSE)
rare_16S + theme(legend.position = "none") + theme_q2r()+ theme(legend.position = "right") + 
  facet_wrap(~fraction, scales="free_y",nrow=4,ncol=3)
ggsave("plots/16S/16S_rarefaction_fractions_7509ASV.pdf", plot = last_plot(), height = 4, width = 6, device = "pdf")

# Estimate minimum, mean, and maximum 16S read counts
...
# Export to CSV
write.csv(summary_table, file = "statistics/16S/summary_table_ps_rare.csv", row.names = FALSE)
...
write.csv(summary_pairwise_alpha_div, "statistics/16S/summary_pairwise_Wilcox_alpha_diversity.csv", row.names = TRUE)
...
ggsave("plots/16S/Shannon_by_fraction.pdf", height = 3, width = 4, device = "pdf") # Adjust width to 4 inches
...
p2
# Save p2 as a PDF
ggsave("plots/16S/Simpson_by_fraction.pdf", height = 3, width = 4, device = "pdf")
...
p3
# Save p3 as a PDF
ggsave("plots/16S/Observed_by_fraction.pdf", height = 3, width = 4, device = "pdf")
...
p4
# Save p4 as a PDF
ggsave("plots/16S/Evenness_by_fraction.pdf", height = 3, width = 4, device = "pdf")
###Put the plots in one image, Figure S2, panel Bacterial community
library(gridExtra)
grid_plot <- grid.arrange(p1, p2, p3, p4, ncol = 2)
ggsave("plots/16S/combined_plots_alpha_div.pdf", grid_plot, width = 4.5, height = 6)  # size of image with all 4 plots!
...
# Inspect the structure of one of the results
str(permanova_results$Bray_Curtis_Fraction)  # Replace with an actual key from the list
...
write.csv(table_data, "statistics/16S/16S_permanova_results_table.csv", row.names = FALSE)
...
print(anosim_table)
# Export the table to a CSV file for further formatting if needed
write.csv(anosim_table, "statistics/16S/16S_anosim_results_table.csv", row.names = FALSE)
...
final_plot
# Save the final plot
ggsave("plots/16S/Combined_nMDS_Plots_Bray_Jaccard_WUnifrac_Unifrac.pdf", final_plot, height = 7, width = 8, device = "pdf")
...
g1 <- g1 +
  ...
  coord_flip() +
  ggtitle("16S 7509 ASV Bray-Curtis Dissimilarity by Fraction")
print(g1)
# Save the plot
ggsave("plots/16S/16S_Bray_box-whiskers_7509ASV_fraction.pdf", plot = g1, height = 4, width = 6, device = "pdf")
...
g_depth <- g1_depth +
  ...
  coord_flip() +
  ggtitle("16S 7509 ASV Bray-Curtis Dissimilarity by Depth")
print(g_depth)
# Save the plot
ggsave("plots/16S/16S_Bray_box-whiskers_7509ASV_depth.pdf", plot = g_depth, height = 4, width = 6, device = "pdf")
...
g1 + 
  ...
  coord_flip() +
  ggtitle("16S 7509ASV Bray-Curtis dissimilarity_ITW")
ggsave(filename = "plots/16S/Bray_box-whiskers_7509features_ITW.eps", plot = last_plot(), device = "eps", path = NULL, scale = 1, width = 4, height = 5, dpi = 300)
ggsave("plots/16S/Bray_box-whiskers_7509features_ITW.pdf", height=2, width=6, device="pdf")
...
# 2. Convert the phyloseq object to a data frame and add ITW condition
ps_melted <- psmelt(ps_rel)

# Create two subsets: ITW == "yes" and ITW == "no"
...
class_colors_top15 <- c(
  ...
  "#1F78B4", "#FDBF6F", "#FF7F00", "#FFFF99", "#FB9A99", 
  "orchid", "khaki4", "peachpuff","gray50" , "yellow", 
...
#######Unifying colors for matching Classes
...
mismatched_colors_noITW <- setNames(remaining_colors[(length(mismatched_ITW) + 1):length(remaining_colors)], mismatched_noITW)
